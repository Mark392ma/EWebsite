...
export default function Reminders({ reminders = [] }: { reminders: Reminder[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsPending(true);
    try {
      // TODO: Implement reminder creation via Inertia post
      // const response = await router.post('/fee-management/reminders', form);
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  <Button type="submit" disabled={isPending} className="bg-teal-700 hover:bg-teal-800 text-white">
                    <Send className="h-4 w-4 mr-2" /> {isPending ? "Sending..." : "Send Reminder"}
                  </Button>
                </div>
              </form>
            </DialogContent>

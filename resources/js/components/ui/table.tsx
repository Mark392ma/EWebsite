...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(({ className, ...props }, ref) =>(
        <tr
           ref={ref}
           className={cn(
            "border-b transition-colors hover:bg-muted/50 data-[state=selected[&_tr]:bg-muted",
             className
            )}
           {...props}
        />
...
const TableHead = React.forwardRef<HTMLTableCellElement, React.HTMLAttributes<HTMLTableCellElement>>(({ className, ...props }, ref) =>(
    <th
        ref={ref}
        className={cn(
        "h-19 px-2 text-left align-middle font-medium text-muted-foregrund [&:has([role=checkbox])]:pr-0 [&.[role-checkbox]]:translate-y-[2px]", className
    )}
        {...props}
    />
))
